...
$(document).ready(function() {
 ...
 function moveg(){
      ...
      $('.btn'+cnt).css('width','30');   
		...
		$('.visual_img').animate({left:position}, 'slow'); //겔러리 무비의 left값을 움직여라~
  ...
  $('.mbutton').click(function(event){  //각각의 버튼을 클릭한다면...
	...
	if($target.is('.btn1')){  //첫번째 버튼을 클릭했다면...
    	   $('.visual_img').animate({left:0}, 'slow');
                cnt=1;
                direct=1;
	}else if($target.is('.btn2')){ //두번째 버튼을 클릭했다면...
    	   $('.visual_img').animate({left:-2000}, 'slow');
                cnt=2;
	}else if($target.is('.btn3')){ //세번째 버튼을 클릭했다면...
    	   $('.visual_img').animate({left:-4000}, 'slow');
                cnt=3;
	}else if($target.is('.btn4')){  //네번째 버튼을 클릭했다면...
    	   $('.visual_img').animate({left:-6000}, 'slow');
                cnt=4;
                direct=-1;
	}
    ...
    if(onoff==false){
		   onoff=true;
		   $('.ps').css('background','url(images/main_visual_ic_stop.png)');
     }  
  });
  ...
  $('.ps').click(function(){ 
       if(onoff==true){
		   ...
		   $(this).css('background','url(images/main_visual_ic_play.png)');
           onoff=false;   
	   }else{
		  timeonoff= setInterval( moveg , 4000); //play버튼 클릭시
		   $(this).css('background','url(images/main_visual_ic_stop.png)');
		   onoff=true; 
	   }
  });	
